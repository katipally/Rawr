...
const TasksPage = async ({
  ...
  const at = (next: { view?: TaskView; mine?: '1'; q?: string }) =>
    tasksPath(workspace, { view: view.key, ...(mine ? { mine: '1' } : {}), ...(q ? { q } : {}), ...next })
  ...
  const writable = canWrite(session.role, 'task')
  ...
  return (
    <div className="flex h-full min-h-0 flex-col gap-3">
      <IndexHeader
        ...
        currentObject="task"
        objects={[
          ...registry.objects.map((other) => ({ key: other.key, label: other.namePlural, href: objectView(workspace, other.key, 'all') })),
          { key: 'task', label: 'Tasks', href: tasksPath(workspace) },
        ]}
      ...
      <div className="flex flex-wrap items-center gap-2">
        <form className="relative w-full min-w-0 sm:w-56" action={tasksPath(workspace)}>
        ...
        <Link
          href={mine ? tasksPath(workspace, { view: view.key, ...(q ? { q } : {}) }) : at({ mine: '1' })}
          aria-pressed={mine}
      ...
      <TasksTable workspace={workspace} rows={rows} assignees={lookups.users} canWrite={writable} emptyTitle={q ? 'No tasks match the search' : view.empty} />
