...
export const FormsTable = ({ workspace, baseUrl, rows }: { workspace: string; baseUrl: string; rows: FormRow[] }) => {
  ...
  const columns: Column<FormRow>[] = [
    {
      ...
      render: (row) => (
        <span className="flex min-w-0 flex-col py-1">
          <Link href={formsPath(workspace, row.id)} className="truncate" onClick={(event) => event.stopPropagation()}>
            {row.name}
          </Link>
    ...
    { key: 'submissions', header: 'Form submissions', width: 160, align: 'right', render: (row) => row.submissions.toLocaleString() },
    {
      ...
      render: (row) =>
        row.quarantined > 0 ? (
          <Link href={submissionsPath(workspace, { state: 'quarantined', form: row.id })} onClick={(event) => event.stopPropagation()}>
  ...
  return (
    <>
      <DataTable
        ...
        rows={rows}
        rowKey={(row) => row.id}
        caption="Forms in this workspace"
        onRowClick={(row) => router.push(formsPath(workspace, row.id))}
      />
      <Modal open={embedding !== null} title={embedding ? `Embed “${embedding.name}”` : 'Embed'} onClose={() => setEmbedding(null)}>
        {embedding ? <EmbedSnippet baseUrl={baseUrl} formId={embedding.id} workspace={workspace} slug={embedding.slug} open /> : null}
      </Modal>
    </>
  )
}
