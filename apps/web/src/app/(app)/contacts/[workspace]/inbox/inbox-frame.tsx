...
export const InboxFrame = async ({
  ...
  const views: { key: string; label: string; count: number; filters: InboxFilters }[] = [
    { key: 'all', label: 'All open', count: counts.all, filters: { scope: 'all', mailboxId: null, unreplied: false, unread: false, q: null } },
    { key: 'mine', label: 'Assigned to me', count: counts.mine, filters: { scope: 'mine', mailboxId: null, unreplied: false, unread: false, q: null } },
    ...
    <div className="flex h-full min-h-0">
      <aside className="hidden w-[14.75rem] shrink-0 flex-col border-r border-line bg-surface md:flex">
        <div className="flex flex-col gap-3 px-4 pt-4 pb-2">
          ...
          <form action={inboxPath(workspace)} className="relative">
        ...
        <nav aria-label="Inbox views" className="flex min-h-0 flex-1 flex-col overflow-y-auto px-2">
          <ul className="flex flex-col">
            {views.map((view) => (
              <li key={view.key}>
                <Link
                  href={inboxPath(workspace, toParams({ ...view.filters, q: filters.q }))}
            ...
            <>
              ...
              <ul className="flex flex-col">
                {mailboxes.map((box) => (
                  <li key={box.id}>
                    <Link
                      href={inboxPath(workspace, toParams({ ...filters, mailboxId: filters.mailboxId === box.id ? null : box.id }))}
