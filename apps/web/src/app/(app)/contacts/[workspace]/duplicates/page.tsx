...
const DuplicatesPage = async ({
  ...
  if (!canWrite(session.role, object)) {
    return (
      <EmptyState
        title="Merging is not something your role does"
        description={`Your role (${session.role}) can read records but not join two of them together.`}
      />
    )
  }

  const pairs = await findDuplicates(contextFrom(session), object)
        ...
        items={[
          {
            key: 'contact',
            label: 'Contacts',
            href: duplicatesPath(workspace, 'contact'),
            current: object === 'contact',
          },
          {
            key: 'company',
            label: 'Companies',
            href: duplicatesPath(workspace, 'company'),
