...
const NewsletterPage = async ({ params }: { params: Promise<{ workspace: string }> }) => {
  ...
  const allowed = canWrite(session.role, 'segment')
            ...
            {events.map((event) => (
              ...
              >
                <span className="min-w-0">
                  <Link href={recordPath(workspace, event.entityType, event.entityId)}>{event.entityName}</Link>{' '}
