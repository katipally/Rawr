...
export const NewsletterPanel = ({
  ...
  if (!canWrite) {
    return (
      <p className="text-secondary">
        You need {hub} access to push an audience to Brevo.
      </p>
    )
  }
  ...
  const push = async () => {
    ...
    setResult(null)
    try {
      const outcome = await api.integrations.pushSegment.mutate({
        segmentId,
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="flex flex-wrap items-end gap-3">
        ...
        <Button variant="primary" busy={busy} disabled={!segmentId} onClick={() => void push()}>
          Push to Brevo
        </Button>
      </div>
