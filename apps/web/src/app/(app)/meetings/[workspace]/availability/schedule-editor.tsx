...
export const ScheduleEditor = ({
  ...
  const [overrideFrom, setOverrideFrom] = useState('09:00')
  const [overrideTo, setOverrideTo] = useState('13:00')
  ...
  const addOverride = async () => {
    try {
      await api.booking.saveOverride.mutate({
        ...
        blocks: overrideKind === 'off' ? [] : [[overrideFrom, overrideTo]],
      })
      show('success', 'Saved.')
      router.refresh()
    } catch (cause) {
      ...
      <section className="rounded-panel border border-line bg-surface p-3 sm:p-4">
        ...
        {editable ? (
          <div className="flex flex-wrap items-end gap-2">
            ...
            {overrideKind === 'hours' ? (
              <>
                <Field id="override-from" label="From">
                  <TextInput
                    id="override-from"
                    type="time"
                    value={overrideFrom}
                    className="w-auto"
                    onChange={(event) => setOverrideFrom(event.target.value)}
                  />
                </Field>
                <Field id="override-to" label="To">
                  <TextInput
                    id="override-to"
                    type="time"
                    value={overrideTo}
                    className="w-auto"
                    onChange={(event) => setOverrideTo(event.target.value)}
                  />
                </Field>
              </>
            ) : null}
        ...
        <section className="rounded-panel border border-line bg-surface p-3 sm:p-4">
          ...
          <ul className="flex flex-wrap gap-2 text-sm">
            {hostedPages.map((page) => (
              <li key={page.id}>
                <Link href={bookingPagesPath(workspace, page.id)}>{page.name}</Link>
