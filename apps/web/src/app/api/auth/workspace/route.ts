...
/** Switches the signed-in workspace so a link into another tenant opens where it
...
export const GET = async (request: NextRequest): Promise<NextResponse> => {
  ...
  if (wanted === session.workspaceSlug) {
  ...
  if (!target) {
    ...
    back.searchParams.set(
      'error',
      `That link belongs to a workspace called "${wanted}", and this account is not a member of it.`,
    )
    return NextResponse.redirect(back)
  }
